...
<div class="mb-6">
  ...
  `});function O(e){let t=e.replace(/[-/\\^$*+?.()|[\]{}]/g,`\\$&`);return t=t.replace(/:(\w+)/g,`([\\w]+)`),t=`^${t}$`,new RegExp(t)}var k=class{constructor(e){this.routes={},this._routesArray=[],this.$container=e,window.addEventListener(`popstate`,this.handlePopState.bind(this))}addRoute(e,t){let n=O(e);this.routes[n]=t,this._routesArray.push(n)}navigateTo(e){history.pushState(null,``,e),this.handleRoute(e)}handlePopState(){this.handleRoute(location.pathname)}handleRoute(e){let t=this._routesArray.find(t=>t.test(e)),n=this.routes[t];n?n():(this.$container.innerHTML=D(),console.log(`404 Not Found`))}};const A=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`})),j=new k(document.querySelector(`#root`)),M=async()=>{j.handleRoute(location.pathname)},N=async()=>{M()};j.addRoute(`/`,async()=>{let e=document.querySelector(`#root`);e.innerHTML=E({loading:!0});let t=await a();e.innerHTML=E({...t,loading:!1})}),j.addRoute(`/product/:productId`,async()=>{let e=document.querySelector(`#root`);e.innerHTML=m({loading:!0});let t=location.pathname.split(`/`)[2],n=await o(t);if(e.innerHTML=m({loading:!1,product:n.error?void 0:n,relatedProducts:[]}),!n.error){let r=(await a({page:1,category2:n.category2})).products.filter(e=>e.productId!==t);e.innerHTML=m({loading:!1,product:n,relatedProducts:r})}}),document.body.addEventListener(`click`,e=>{let t=e.target.closest(`.product-card`)??e.target.closest(`.related-product-card`);if(t){let e=t.dataset.productId;history.pushState(null,null,`/product/${e}`),M()}if(e.target.tagName===`A`){if(e.preventDefault(),location.pathname===e.target.pathname)return;j.navigateTo(e.target.pathname)}}),A().then(N);